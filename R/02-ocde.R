...
rmw <- OECD::get_dataset("RMW",
  ...
  mutate_at(vars(rmw|year), funs(as.numeric(.)))
...
mrw <- OECD::get_dataset("MIN2AVE",
  ...
  mutate_at(vars(mrw|year), funs(as.numeric(.)))
...
emp <- emp %>%
  ...
  mutate_at(vars(starts_with("emp")|year), funs(as.numeric(.)))
...
lfs <- lfs %>% 
  ...
  mutate_at(vars(starts_with("emp")|starts_with("unemp")|starts_with("lfp")|year), funs(as.numeric(.)))
...
pt <- pt %>%
  ...
  mutate_at(vars(starts_with("pt")|year), funs(as.numeric(.)))
...
hrs <- hrs %>%
  ...
  mutate_at(vars(starts_with("hrs")|year), funs(as.numeric(.)))
...
wise <- wise %>%
  ...
  mutate_at(vars(starts_with("emp")|starts_with("pop")|starts_with("gdp")|contains("lp")|
                 gni|tradeopen|hdi|gini|year), funs(as.numeric(.)))
...
gdp <- gdp %>%
  ...
  mutate_at(vars(lp|year),funs(as.numeric(.)))
...
gwg <- gwg %>% 
  ...
  mutate_at(vars(gwg|year),funs(as.numeric(.)))
...
labels <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1aw_byhiC4b_0XPcTDtsCpCeJHabK38i4pCmkHshYMB8/edit#gid=0",
  ...
  filter(grepl("_oecd|year|iso3c", variables))
