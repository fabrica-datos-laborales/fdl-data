# Code: DPI ----------------------------------------------------------
# 1. Install packages -----------------------------------------------------
pacman::p_load(tidyverse, readr, dplyr, car, googlesheets4)
...
dpi <- dpi %>% select(year, country_name = countryname, elec_sys = system, if_military = military, exec_party = execme,
...
labels <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1aw_byhiC4b_0XPcTDtsCpCeJHabK38i4pCmkHshYMB8/edit#gid=0",
                                    range = c("B2:C324"), col_names = F) %>%
  slice(c(1,5,313:323)) %>% # selecciono 1, 2, donde parte -5, donde termina -5
...
## Etiquetar
label(dpi) = as.list(var.labels[match(names(dpi), names(dpi))])

# 7. Save -----------------------------------------------------------------
rm(list = ls(pattern = "wl|b|p|h"))#remover lo que no sirve
saveRDS(dpi, file="output/data/proc/dpi.rds")
